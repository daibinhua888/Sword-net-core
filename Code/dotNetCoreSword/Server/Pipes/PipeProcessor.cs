...
namespace Sword.Server.Pipes
{
    public class PipeProcessor
    {
        ...
        private void InnerProcessLogic()
        {
            while (true)
            {
                //Console.WriteLine(Thread.CurrentThread.ManagedThreadId+"--InnerProcessLogic 111");
                var cmd = this.incomeCommand.Take();
                //Console.WriteLine(Thread.CurrentThread.ManagedThreadId + "--InnerProcessLogic 222");
